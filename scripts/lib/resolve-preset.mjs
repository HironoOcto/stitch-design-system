 ...
 * @returns {string}  consumer's .agent/stitch.theme.json `activeSite`, else publishedDefault
 *
 * The consumer pointer is `<root>/.agent/stitch.theme.json` (the agent dir; deliberately NOT
 * the repo's own `stitch.config.json`, which is this project's build config). A missing /
 * malformed / activeSite-less consumer pointer is not an error — it is the zero-config path:
 * the consumer gets the published default, matching the npm package.
 */
export function resolveActiveSite(consumerRoot, publishedDefault) {
  const cfgPath = resolve(consumerRoot, '.agent', 'stitch.theme.json');
