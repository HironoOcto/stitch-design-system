...
const less = readFileSync(
  'packages/react/src/components/Radio/radio.module.less',
  'utf8',
);
...
describe('Radio a11y', () => {
  it('默认选项组无 axe 违规', async () => {
    const { container } = render(<Radio options={options} defaultValue="a" />);
    expect(await axe(container)).toHaveNoViolations();
  });

  it('group 级 disabled 无 axe 违规', async () => {
    const { container } = render(
      <Radio options={options} disabled defaultValue="a" />,
    );
    expect(await axe(container)).toHaveNoViolations();
  });

  // 规则 7：checked 单选圈用 accent 铺底，与派生自 accent 的 focus-ring 同色 → 须叠间隔环。
