...
interface FormStore {
  ...
  /** 遍历字段元数据 */
  forEachField: (
    cb: (meta: { key: string; rules: Rules; notify: () => void }) => void,
  ) => void;
}
...
function createFormInstance(options: FormOptions): FormInstance {
  ...
  function resetFields(nameList?: NamePath[]): void {
    const keys = nameList
      ? nameList.map(stringifyNamePath)
  ...
  async function validateFields(
    ...
    fieldStore.forEachField((meta) => {
      if (
        !nameList ||
        nameList.some((n) => stringifyNamePath(n) === meta.key)
      ) {
      ...
      targetMetas.map(async (meta) => {
        ...
        for (const r of ruleList) {
          const rule: RuleObject =
            typeof r === 'function' ? r(formInstance) : r;
          try {
  ...
  (
    formInstance as unknown as { __bindCallbacks: typeof bindCallbacks }
  ).__bindCallbacks = bindCallbacks;
...
export function useForm<T = Record<string, unknown>>(
  ...
  // React 官方的「惰性初始化 ref」惯例：首帧建一次稳定实例、后续复用同一引用。
  // 实例是稳定值不参与渲染输出，读写 .current 安全 —— 抑制 react/refs 的渲染期误报。
