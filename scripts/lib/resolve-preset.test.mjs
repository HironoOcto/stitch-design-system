...
// consuming project's stitch.config.json `activeSite` picks which embedded preset
...
// A throwaway consuming-project root with a given stitch.config.json content.
function consumerRoot(configText) {
  const dir = mkdtempSync(join(tmpdir(), 'consumer-'));
  if (configText !== null)
    writeFileSync(join(dir, 'stitch.config.json'), configText);
  return dir;
}
...
test('no stitch.config.json → falls back to the published default', () => {
...
test('malformed stitch.config.json → falls back (never throws on a bad consumer file)', () => {
