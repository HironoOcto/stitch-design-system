...
export function buildTokens({ site, out } = {}) {
  ...
  // Page-scale layer (ADR 0012): the self-proving dist/tokens.css folds it in so the
  // merged :root carries the full --stitch-space-* / text scale / layout keys and the
  // contract's aliased spacing (var(--stitch-space-N, …)). Generated by build:layout.
  const layer = resolve(root, 'sites', resolvedSite, 'layout.css');
  const adapter = resolve(root, 'sites', resolvedSite, 'adapter.css');
  const outPath = resolve(root, out || 'packages/tokens/dist/tokens.css');
  const css = mergeTokens(contract, layer, adapter, resolvedSite);
