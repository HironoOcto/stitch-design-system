// 测试辅助组件（Capturer/Bind/Spy）刻意在渲染期把 form 实例/spy 抓到外层变量，
// 供断言读取——是测试惯例、非生产副作用，整文件抑制 react/globals。
...
describe('Form', () => {
  ...
  describe('布局', () => {
    it('layout=vertical 时 FormItem 渲染为 block', () => {
      ...
      expect(
        container.querySelector(`.${styles.vertical}`),
      ).toBeInTheDocument();
    });
    ...
    it('layout=horizontal 时使用 grid 布局', () => {
      ...
      expect(
        container.querySelector(`.${styles.horizontal}`),
      ).toBeInTheDocument();
    });
    ...
    it('layout=inline 时 Form 渲染为 flex', () => {
      ...
      expect(container.querySelector(`.${styles.inline}`)).toBeInTheDocument();
    });

    it('size=small 应用 form-small 类', () => {
      const { container } = render(
        <Form size="small">
          <Form.Item name="s">
            <Input />
  ...
  describe('校验', () => {
    ...
    it('type=integer 规则：字符串数字按整数校验', async () => {
      ...
      expect(onFinish).not.toHaveBeenCalled();
      expect(
        container.querySelector('.' + styles['item-explain-error']),
      ).toBeTruthy();
    ...
    it('min/max 规则：number/integer 类型按数值比较（不受字符串长度影响）', async () => {
      ...
      const { container } = render(
        <Form onFinish={onFinish}>
          <Form.Item
            name="age"
            rules={[{ type: 'integer', min: 0, max: 150, message: '0-150' }]}
          >
            <Input />
          </Form.Item>
        </Form>,
    ...
    it('pattern 规则：正则校验', async () => {
      ...
      const { container } = render(
        <Form onFinish={onFinish}>
          <Form.Item
            name="email"
            rules={[{ pattern: /^[a-z]+@/i, message: '邮箱格式不对' }]}
          >
            <Input />
          </Form.Item>
        </Form>,
      ...
      const { container } = render(
        <Form onFinish={onFinish}>
          <Form.Item
            name="name"
            rules={[
              { required: true, whitespace: true, message: '不能为空或空白' },
            ]}
          >
            <Input />
          </Form.Item>
  ...
  describe('label / colon / requiredMark', () => {
    ...
    it('colon=false 时不显示冒号', () => {
      ...
      expect(
        container.querySelector('.' + styles['item-label-colon']),
      ).toBeNull();
    });
    ...
    it('requiredMark=false 时不显示星号', () => {
      ...
      expect(
        container.querySelector('.' + styles['item-label-required']),
      ).toBeNull();
    });
  ...
  describe('高级功能', () => {
    it('noStyle 时不渲染外层 label/wrapper 容器', () => {
      ...
      expect(container.querySelector('.' + styles['item-label'])).toBeNull();
      expect(container.querySelector('.' + styles['item-control'])).toBeNull();
      // input 还在
      expect(screen.getByRole('textbox')).toBeInTheDocument();
    });

    it('hasFeedback 时错误状态下显示反馈图标（走 <Icon>）', async () => {
      const { container } = render(
        <Form>
          <Form.Item
            name="a"
      ...
      const feedback = container.querySelector(
        '.' + styles['item-feedback-icon'],
      );
      expect(feedback).toBeInTheDocument();
      // 图标走 <Icon name="close">，无 emoji / Unicode 符号
      expect(feedback).toHaveAttribute('data-icon', 'close');
    });
    ...
    it('validateStatus=success 手动指定覆盖自动推断', () => {
      ...
      expect(
        container.querySelector('.' + styles['item-has-success']),
      ).toBeInTheDocument();
    });
      ...
      render(
        <Form>
          <Form.Item
            name="a"
            help="帮助说明"
            rules={[{ required: true, message: '错误文案' }]}
          >
            <Input />
          </Form.Item>
        </Form>,
      ...
      function CustomInput({
        ...
        return (
          <input
            value={value ?? ''}
            onChange={(e) =>
              onChange?.((e.target as HTMLInputElement).dataset.v ?? '')
            }
            data-v="from-custom"
          />
        );
        ...
        return (
          <>
            <button
              ...
              onClick={() => {
                form.setFields([
                  { name: 'a', errors: ['服务器报错'], touched: true },
                ]);
              }}
            >
              setErr
