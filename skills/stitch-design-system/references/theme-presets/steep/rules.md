...
- 中性梯（steep 4 档灰 → 收敛契约 3 档）：ink-black #17191c（标题/强调；内联文字链接亦归此近黑，见「组件规格 · Text Link」）、slate-gray #777b86（次要 helper / footer）、smoke-gray #a3a6af（占位符/弱化 = text-muted，disabled 由其派生，见「组件规格 · Input」）；ash-gray #979799 是三级标签 / 分类 tag（契约 3 档文字槽无独立位，见「组件规格 · Tag / Category Label」）。
