...
- **`build:blurb` 吸收合成层修正**（#36 落地）：合成层若与 `DESIGN.md` 冲突，须有机制把真相带进 skill 的 style-paragraph——否则 `style.md`（继承 austere 的 DESIGN 框）与 `composition.md`（真站 rich 真相）长期矛盾。落点是 **`build:blurb`（整条链唯一的 LLM 步）**：`composition.md` 成其**可选融合输入**（`extractComposition` = `stripTrace(源)` 的 consumer-clean 正文），prompt 融合「DESIGN 概览三段 + 真站修正 → 一段忠于真站的 style」，**冲突以 composition 为准**；`skill-blurb.md` 头注契约升级为 **`faithful to DESIGN.md as corrected by composition.md`**。缺 `composition.md` 则退化为只读 `DESIGN.md` 的原行为。`build:skill` **仍纯确定、不引入 LLM**（只读冻结的 `skill-blurb.md`）；冻盘 / 重生成产物是下游人审步（#37），非本机制。
