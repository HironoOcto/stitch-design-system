...
function defaultGetValueFromEvent(event: unknown): StoreValue {
  ...
  const target = (
    event as { target?: { value?: unknown; checked?: unknown; type?: string } }
  ).target;
...
export const FormItem: React.FC<FormItemProps> = (props) => {
  ...
  const fieldKey = useMemo(
    () => (name !== undefined ? stringifyNamePath(name) : null),
    [name],
  );
  ...
  const computedStatus =
    validateStatus ??
    (isValidating ? 'validating' : errors?.[0] ? 'error' : '');
  ...
  const isRequired =
    required || rules.some((r) => (typeof r === 'object' ? r.required : false));
  ...
  if (fieldKey && childIsElement) {
    ...
    childProps[trigger] = (event: unknown) => {
      ...
      const finalValue = normalize
        ? normalize(rawValue, prevValue, form.getFieldsValue(true))
        : rawValue;
  ...
  if (
    helpId &&
    childIsElement &&
    childProps['aria-errormessage'] === undefined
  ) {
    childProps['aria-errormessage'] =
      computedStatus === 'error' ? helpId : undefined;
  }
  ...
  // status 透传：Input/Select 等控件支持 error / warning 两种校验态边框，
  // 两者都透传（success / validating 非控件边框态，仅由 explain 文案着色体现）。
  if (
    childProps.status === undefined &&
    (computedStatus === 'error' || computedStatus === 'warning')
  ) {
    childProps.status = computedStatus;
  }

  const renderChildren =
    fieldKey && childIsElement
      ? React.cloneElement(children, childProps)
      : children;

  // 布局：inline 模式下 FormItem 退化为 vertical（每个 item 独占一行）
  const itemLayoutTyped = (itemLayout ?? ctxLayout) as
    'horizontal' | 'vertical' | 'inline';
  const layout: 'horizontal' | 'vertical' | 'inline' =
    itemLayoutTyped === 'inline' ? 'vertical' : itemLayoutTyped;
  ...
  const labelEndCol =
    (mergedLabelCol?.span ?? 0) + (mergedLabelCol?.offset ?? 0);
  ...
  const rootCls = clsx(
    ...
    styles[`item-${size}`],
    {
      [styles['item-has-error']]: computedStatus === 'error',
      [styles['item-has-warning']]: computedStatus === 'warning',
      [styles['item-has-success']]: computedStatus === 'success',
      [styles['item-is-validating']]: computedStatus === 'validating',
      [styles['item-required']]: showRequiredMark,
    },
    className,
  );
  ...
  const labelNode =
    label !== undefined ? (
      <label
        ...
        className={clsx(styles['item-label'], {
          [styles['item-label-required']]: showRequiredMark,
          [styles['item-label-colon']]: showColon && label !== '',
        })}
  ...
  const helpNode =
    showHelp !== undefined ? (
      <div
        id={helpId}
        // 错误态：作为 live region 播报（与 child input 的 aria-errormessage 配对，
        // 满足屏幕阅读器「错误须被主动通知」契约）
        role={computedStatus === 'error' ? 'alert' : undefined}
        className={clsx(styles['item-explain'], {
          [styles['item-explain-error']]: computedStatus === 'error',
        })}
      >
        {hasFeedback && computedStatus === 'error' ? (
          <Icon
            name="close"
            size={14}
            className={styles['item-feedback-icon']}
          />
        ) : null}
        {showHelp}
      </div>
  ...
  return (
    <div className={rootCls} data-field-name={fieldKey ?? undefined}>
      {labelNode}
      <div className={styles['item-control']} style={wrapperColStyle}>
        <div className={styles['item-control-input']}>{renderChildren}</div>
        {helpNode}
      </div>
    </div>
  );
