...
> 当前生效主题（`activeSite`）的 skill「**按规格造好、没坏**」的结构验收标准。认证结构，**不**认证「好不好用」——触发/产出质量的 eval 另立、晚一步做。
...
> **`<当前站>` = `stitch.config.json` 的 `activeSite`**（判据里的占位符）。全程**不写死某站**。
>
...
0. **产物存在**：9 个产物文件都在各自路径——缺任一 = 该产物节直接失败（验收不为补齐而跑 build）。
...
| 产物 | 修复改哪 | 重验 |
|---|---|---|
| **生成物**：`references/components/*`、`theme/*`、SKILL.md 的槽、README catalog 槽 | 改**源**（组件源码 / adapter / rules / DESIGN.md / `component-families.md`） | 重跑对应命令（`build:refs` 或 `build:skill`）再验；幂等，修复在源里、不会被覆盖 |
| **手写 / 手审件**：SKILL.md 骨架(非槽)、README 骨架(非槽)、`react-project.md`、`standalone-html.md`、`skill-blurb.md` | **直接改**该文件 | 直接重验；`build:skill` 只替换槽区 / 只读 blurb，碰不到你改的部分 |
...
| `description` 长度合法 | 非空且 ≤ 1024 字符 | 机器 | 空 description；或 1500 字长文 |
...
| 文件引用只一层深 | 所有链接目标匹配 `references/[^/]+(/[^/]+)?`，无 `references/a/b/c` 深链 | 机器 | `references/theme/sub/deep.md` |
| 自包含·无结构残留 | grep 无 `raw.githubusercontent`/`github.com`/出 skill 的 `../`/外来包名/`--(?!stitch)[a-z]+-` 前缀 | 机器 | 出现 GitHub raw 链、`--ant-*`、外来包名 |
| 两 blurb 处已注入 | description 与 style-paragraph 两处含主题文案、无占位残留（占位形如 `【槽…】`）；注入的标记机制以 [pipeline §4.0/§4.4](./skill-build-pipeline.md) 为准 | 机器 | 还留 `【槽①：当前主题风格词】` 占位 |
...
| blurb 两段 == 冻盘逐字 | SKILL 注入的两段 diff `sites/<当前站>/skill-blurb.md` 两段 == 空 | 机器 | SKILL 手改了注入文本、与源漂移 |
| 无他站主题残留（词汇层） | LLM 读 SKILL.md：除**当前站** vocab 外，无他站/来源 bundle 的品牌·字体·风格词（denylist = 所有非 active 站的词）。**当前站自己的风格词是应有的、不算残留** | agent判 | 混进别的站的字体名 / 风格词 |
| description 是「何时用」导向 | LLM 读 description：含 what+when、有视觉触发关键词、第三人称、无营销形容词（[Optimizing descriptions](https://agentskills.io/skill-creation/optimizing-descriptions)） | agent判 | `description: Helps with UI.`（无 when、无关键词） |
...
### 6. `references/theme/tokens.css`（生成 · 随主题）

| 查什么(白话) | 怎么算过(命令/grep/diff 或判据) | 类型 | 不过长啥样(失败例子) |
|---|---|---|---|
| == contract + 当前 adapter 合并 | `mergeTokens(contract, 当前站 adapter, 当前站)` 的输出（含头部 `generated for site: … DO NOT EDIT` 行 + `:root` 体）== 产物内容（纯函数算期望、比对；**不跑 build:skill、不动 git**） | 机器 | 手改 hex、与合并结果不符 |
...
### 7. `references/theme/design-rules.md`（生成 · 拷贝 · 全局）

| 查什么(白话) | 怎么算过(命令/grep/diff 或判据) | 类型 | 不过长啥样(失败例子) |
|---|---|---|---|
| == 源逐字（全局源、拷贝无加工） | `diff SKILL_DIR/references/theme/design-rules.md docs/design-system/design-rules.md` == 空（源是全局的、与主题无关，故这份拷贝天然主题无关） | 机器 | 副本被手改、与源漂移 |

### 8. `references/theme/rules.md`（生成 · 拷贝 · 当前主题）

| 查什么(白话) | 怎么算过(命令/grep/diff 或判据) | 类型 | 不过长啥样(失败例子) |
|---|---|---|---|
| == 当前站 rules 逐字 | `diff SKILL_DIR/references/theme/rules.md sites/<当前站>/rules.md` == 空 | 机器 | 副本手改、与 `sites/<当前站>/rules.md` 不符 |

### 9. `sites/<当前站>/skill-blurb.md`（生成一次 · 人审 · 主题 blurb 源）

| 查什么(白话) | 怎么算过(命令/grep/diff 或判据) | 类型 | 不过长啥样(失败例子) |
|---|---|---|---|
| 两段结构齐 | grep 含 `## description` 与 `## style-paragraph` | 机器 | 只有一段、或标题拼错 |
| description ≤ 1024·无 hex | `## description` 段字符 ≤ 1024 且 grep 无 `#[0-9a-fA-F]{3,8}` | 机器 | 段里写死 `#fbe1d1` |
| style 段一段·无 hex·只招牌尺寸 | `## style-paragraph` 无 hex、只留 hero 字号/卡片圆角/区块间距几个尺寸 | 机器 | 复述整张 token 表 |
| 忠实 DESIGN.md、无编造、描述为 THE style | LLM 读 blurb vs `sites/<当前站>/source/DESIGN.md`：每句可溯源、没借别站、不说「one of several / switchable」 | agent判 | blurb 编了 DESIGN.md 没有的视觉词 |
| 人最后定稿签字 | 顶部注释含 human-approved；维护者一眼过（唯一人工闸） | 人签 | 未审直接发布 |
