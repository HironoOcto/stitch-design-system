...
| **`build:skill`** | 为每个可发布站备预置 `theme-presets/<站>/{tokens.css,rules.md,style.md}` + 全局 `theme/design-rules.md` + 注入 SKILL.md `SLOT:default-site`（[ADR 0010](../adr/0010-consume-time-theme-choice.md)；`tokens.css` 为 `mergeTokens(contract, layer, adapter)` **三输入折入页面尺度层**，[ADR 0012](../adr/0012-page-scale-layer.md) 决策5；旧版单套见 §4.1–4.4） | ✅ 确定，纯文件操作 | 改可发布站源 / 换发布默认 / 发布前 |
