...
skill 不再在 `build:skill` 时把 `activeSite` 那**单套**主题烤进 `references/theme/*`。改为：`build:skill` 为**每个可发布站**（[listPublishableSites](../../scripts/lib/publishable-sites.mjs)，#7 三件套判据）各生成一套预置 `references/theme-presets/<站>/{tokens.css, rules.md, style.md}`；「哪套生效」的决定权从**发布时**搬到**消费时**——消费项目的 `.agent/stitch.theme.json` 的 `activeSite` 指针在 skill **被读时**解析，选中对应预置；无指针则回落**发布默认**（= `build:skill` 当时的 `activeSite` = npm 包默认皮，[ADR 0007](./0007-active-site-single-switch.md)），两侧默认同名对齐。

> **消费侧指针文件（修订）**：消费项目的指针是 `<项目根>/.agent/stitch.theme.json`（`.agent/` 是通用 agent 目录、也是本插件安装所在）。它**刻意区别于本仓库自己的 `stitch.config.json`**——后者是开发本设计系统时的构建配置（定发布默认，被 vite 插件 / build:skill / build:tokens 读），前者是装了插件的使用方选主题的指针（reset-theme 写、stitch-design-system skill 读）。两者不同名、不同位、不同角色,避免混淆。
...
- **[ADR 0007](./0007-active-site-single-switch.md)（`activeSite` 单一开关）**：`activeSite` **细化为两层指针（两个不同文件）**——① 本仓库根 `stitch.config.json` 的 `activeSite` = **发布默认**，驱动 `vite build` 的 `dist/style.css` 与 `build:skill` 注入的 SKILL.md `SLOT:default-site`（两条构建仍读同一文件、仍共用 `mergeTokens`）；② 消费项目 `.agent/stitch.theme.json` 的 `activeSite` = **消费侧指针**，读时解析选预置。两层是**不同文件**（角色不同：构建配置 vs 消费指针），对齐的是 `activeSite` 的**值（站名）**：预览侧（`data-site`，[ADR 0009](./0009-themes-preview-export.md)）与开发侧（指针）填同一站名即整体对齐。「一个字段翻转两产物一致」在本仓库内仍成立；对外多了一层「消费项目可自定」。
...
- 与 [ADR 0009](./0009-themes-preview-export.md) 合起来给出完整回路：**预览侧** `import themes/<站>` + 翻 `data-site`，**开发侧** 改 `.agent/stitch.theme.json` 指针 + skill 读时解析——两侧同一站名即对齐（写指针的 reset-theme skill 见 #10）。
