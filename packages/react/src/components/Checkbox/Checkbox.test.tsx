...
const setup = (props: Partial<CheckboxProps> = {}) => {
  ...
  const utils = render(
    <Checkbox options={baseOptions} onChange={onChange} {...props} />,
  );
...
} & Omit<CheckboxProps, 'value' | 'defaultValue' | 'onChange' | 'options'> &
...
describe('Checkbox', () => {
  describe('rendering', () => {
    it('渲染所有选项的 label 与对应 checkbox 输入', () => {
      ...
      baseOptions.forEach((o) => {
        ...
        // a11y 契约：label 文本必须绑定到 input（用 within(group) 限定避免歧义）
    ...
    it('每个 input 拥有稳定且互不冲突的 id', () => {
      ...
      ids.forEach((id) => expect(id).toMatch(/^stitch-cbx-/));
    });
  });
  ...
  describe('controlled 模式', () => {
    ...
    it('受控模式下传入 defaultValue 应被忽略', () => {
      render(
        <Checkbox options={baseOptions} value={['b']} defaultValue={['a']} />,
      );
      const inputs = screen.getAllByRole('checkbox') as HTMLInputElement[];
      expect(inputs[0]).not.toBeChecked();
      expect(inputs[1]).toBeChecked();
